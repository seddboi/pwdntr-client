...
export function SideMenu({open, onClose, isHome}) {
	...
	const logout = async () => {
		await Axios.post('http://localhost:3001/logout', {
			...
			JSON.parse(localStorage.removeItem('aT'));
		})
	};
			...
			<Drawer open={open} onClose={onClose} PaperProps={{sx:{color: 'white', backgroundColor: '#17202A'}}}>
				<List>
					<ListItem disablePadding>
						<ListItemButton onClick={() => {navigate(pageSelector, {state: {status: isLoggedIn, user: {id: location.state.user.id, username: location.state.user.username} }})}}>
