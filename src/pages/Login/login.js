		...
		<ThemeProvider theme={theme}>
			<Box sx={{display: 'flex', flexDirection:'column', justifyContent: 'center', alignItems: 'center'}}>
				...
				<Box component='form' sx={{display: 'flex', flexDirection: 'column', width: '90%',maxWidth: '500px', m: 4}}>
					...
					<TextField id="login-username" autoComplete='username' label="Username" error={loginError} variant="outlined" onChange={(e) => {setLoginUsername(e.target.value); setLoginError(false); setLoginErrorMessage('')}} sx={{mt: 2, mb: 2}}/>		
					<TextField id="login-password" autoComplete='password' label="Password" error={loginError} type={showPassword ? 'text' : 'password'} variant="outlined" onChange={(e) => {setLoginPassword(e.target.value); setLoginError(false); setLoginErrorMessage('')}} InputProps={{
						endAdornment: 
					...
					<Typography id='submitMessage' variant='h6' sx={{color: 'red', mb: 8}}>{loginErrorMessage}</Typography>
				</Box>
